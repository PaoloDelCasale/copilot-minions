...
The subprocess session is not persisted or resumable. The parent extension persists only lifecycle snapshots and renders them in a worker widget, so completed or reload-interrupted work remains visible without treating a dead RPC process as resumable.
