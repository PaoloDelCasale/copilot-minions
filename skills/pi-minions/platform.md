...
The extension also enforces fifty launches. After forty triaged results it accepts
only already-boarded closure work whose spawn sets `budgetClass: "closure"`; after
fifty results it rejects every new dispatch.
...
retrying the worker or changing its role. Worker/run cost ceilings and native duration
